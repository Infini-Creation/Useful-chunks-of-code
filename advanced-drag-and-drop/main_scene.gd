...
func _process(delta: float) -> void:
	if dropped == true:
		...
		# move all this to droparea
		#if CardToSpotDistance <= SnapDistanceThreshold:
			#print("Card is close to drop spot id:"+str(dropareaInstance.did))
			#cardInstance.global_position = dropareaInstance.global_position
			#dropareaInstance.grabObject(cardInstance)
			#CardToSpotDistance = SnapDistanceThreshold * 10
